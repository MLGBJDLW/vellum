// ============================================
// Spec Tasks Agent Definition
// ============================================
// T021: Tasks agent for spec workflow

import { AgentLevel } from "../../agent/level.js";
import type { CustomAgentDefinition } from "../custom/types.js";

/**
 * System prompt for spec tasks agent.
 *
 * Guides the agent in task decomposition and planning.
 */
const TASKS_SYSTEM_PROMPT = `You are a Spec Tasks Planner - a specialized agent focused on task decomposition and implementation planning.
 ...
 * Spec Tasks Agent Definition.
 *
 * Level 2 worker specialized in task decomposition and planning
 * for the spec workflow. Creates implementable task lists.
 *
 * @example
 * ```typescript
 * import { specTasksAgent } from './spec/tasks.js';
 *
 * registry.register(specTasksAgent);
 * ```
 */
export const specTasksAgent: CustomAgentDefinition = {
  // Identity
  slug: "spec-tasks",
  ...
  // LLM Configuration
  systemPrompt: TASKS_SYSTEM_PROMPT,
  ...
  whenToUse: {
    ...
    triggers: [
      { type: "keyword", pattern: "tasks|decompose|breakdown" },
      { type: "keyword", pattern: "planning|milestone|sprint" },
    ],
    priority: 10,
  },

  // Metadata
  tags: ["spec", "tasks", "planning", "decomposition"],
  version: "1.0.0",
  author: "vellum",
};
