...
/**
 * Get onboarding config file path
 */
function getOnboardingConfigPath(): string {
  return path.join(getVellumDir(), "config.toml");
}
...
export class OnboardingWizard {
  ...
  static hasConfig(): boolean {
    return fs.existsSync(getOnboardingConfigPath());
  }
...
export async function shouldRunOnboarding(): Promise<boolean> {
  ...
  // Missing config should re-run onboarding
